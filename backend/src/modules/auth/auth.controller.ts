...
const signup = asyncHandler(async(req: Request, res: Response) => {
    ...
    const {accessToken, refreshToken} = await sessionService.createSession({userId: user.userId})
...
const forgotUsername = asyncHandler(async(req: Request, res: Response) => {
    ...
    return res.status(200).json(new ApiResponse(200, {}, "If an account exists for this email, you will receive a message shortly"))
});
...
const refreshAccessToken = asyncHandler(async(req: Request, res: Response) => {
    const {refreshToken: oldRefreshToken} = authSchema.refreshTokenValidationSchema.parse({
        refreshToken: req.cookies.refreshToken
    });

    const {accessToken, refreshToken: newRefreshToken} = await sessionService.refreshAccessToken(oldRefreshToken);
